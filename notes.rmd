...
```
  ...
  + See that most players clearly don't belong to the Hall of Fame, a few do without any doubt and then there are some inbetween.
...
```
  ...
  + Note, that simply calculating the cummulative mean achieves the same results

#### 5.4 Q-values

  + We can start adding each player one by one and calculate the FDR at each new inclusion. This way we can calculate the cummulative mean PEP at each threshold, aka. the **q-value**.
